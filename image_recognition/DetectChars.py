...
def loadKNNDataAndTrainKNN():
    ...
    classifications = os.path.join(sys.path[0], "image_recognition/classifications.txt")
    flattened_images = os.path.join(sys.path[0], "image_recognition/flattened_images.txt")

    try:
        npaClassifications = np.loadtxt(classifications, np.float32)                  # read in training classifications
    except Exception as e:
        print(e)# if file could not be opened
    ...
    try:
        npaFlattenedImages = np.loadtxt(flattened_images, np.float32)                 # read in training images
